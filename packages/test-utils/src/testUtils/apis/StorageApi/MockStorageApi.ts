...
export class MockStorageApi implements StorageApi {
  static factory = {
    ...
    deps: {},
    factory: () => new MockStorageApi(''),
  };

  private readonly namespace: string;
  private readonly data: Map<string, any>;

  constructor(namespace: string, data?: Map<string, any>) {
  ...
  get<T>(key: string): T | undefined {
    return this.data.get(this.getKeyName(key));
  }

  async set<T>(key: string, data: T): Promise<void> {
    this.data.set(this.getKeyName(key), data);
    this.notifyChanges({ key, newValue: data });
  }

  async remove(key: string): Promise<void> {
    this.data.delete(this.getKeyName(key));
